...
def wrap_langchain_tool(
    ...
    *,
    ...
    Supports common LangChain tool shapes such as StructuredTool/BaseTool
    instances that expose a name, description, optional args_schema, and one
    or more execution methods like ``ainvoke()``, ``arun()``, ``invoke()``, or
    ``run()``.
    """
...
def wrap_autogen_tool(
    ...
    *,
    ...
    Supports common AutoGen function tool shapes that expose a JSON schema via
    ``schema`` and execute through ``run_json()`` or a bound function stored on
    the tool object.
    """
...
def wrap_openai_tool_schema(
    *,
    ...
    """Wrap an OpenAI function/tool schema plus handler as an agenticli command.

    Use this when you only have an OpenAI-compatible schema definition and need
    to supply the execution callable separately.
    """
